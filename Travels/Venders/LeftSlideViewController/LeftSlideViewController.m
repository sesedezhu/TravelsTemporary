...
@property (nonatomic,strong) UIView *shadowView ;//主界面，侧滑时显示阴影效果;
...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    if (needMoveWithTap && (rec.view.frame.origin.x >= 0) && (rec.view.frame.origin.x <= (kScreenWidth - kMainPageDistance)))
    {
        ...
        self.shadowView.frame = rec.view.frame;
    }
    else
    {
        //超出范围，
...
- (void)openLeftView;
{
    ...
    self.shadowView.frame = self.mainVC.view.frame;
...
-(UIView *)shadowView{
    if (!_shadowView) {
        ...
        [self.leftVC.view addSubview:_shadowView];
        _shadowView.frame = self.mainVC.view.frame;
        
        _shadowView.layer.shadowOffset = CGSizeZero;
        _shadowView.layer.shadowOpacity = 0.5;//设置阴影透明度
        _shadowView.layer.shadowColor   = [UIColor blackColor].CGColor;//设置阴影颜色
        _shadowView.layer.shadowRadius  = 5;//设置阴影半径
    }
    return _shadowView;
}
